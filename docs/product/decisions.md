...
| DEC-038 | 2026-09-17 | Decidida | Nenhuma foto ilustrativa com marca d'água ou identificação visual de negócio terceiro poderá ser referenciada pelo código do protótipo ou do MVP; somente fotos sem essa marca são elegíveis. | Marca d'água de um negócio nomeado ("Açaí no Ponto") foi identificada em pelo menos 3 das 6 fotos enviadas ao v0 em 17/09/2026 fora desta sessão (ver `prototypes/web/VERIFICATION.md`), o que indica autoria de terceiro incompatível com uso no produto. A foto atualmente em uso (`acai-tradicional.webp`) não apresenta essa marca e permanece; as demais 6 fotos não foram trazidas ao repositório e não serão referenciadas no código sem confirmação individual de ausência de marca de terceiro. |
| DEC-039 | 2026-09-17 | Decidida | Incorporado ao protótipo o feedback de usabilidade do responsável pelo projeto (registrado no Notion em 17/09/2026, página "Protótipo Açaí Conecta"), válido para os 3 níveis de acesso: (1) o protótipo deve se apresentar como um produto íntegro, sem expor rótulos redundantes de "fictício/simulado/demonstração" na interface visível ao usuário; (2) o catálogo de produtos deve ter como padrão do sistema apenas dois tamanhos, 500 ml e 1.000 ml (1 L); (3) cada tela deve ter no máximo um ponto de entrada de "Preciso de ajuda" por contexto, sem repetição com o mesmo significado; (4) o acesso de operador e administrador deve ocorrer por uma tela de login própria antes do painel, nunca como sobreposição a uma tela já carregada em segundo plano; (5) botões de ação devem seguir boas práticas de UI/UX para parecerem claramente clicáveis/navegáveis, incluindo indicação visual de estado (ex.: verde/vermelho para batedeira aberta/fechada) e confirmação antes de alternar o estado de abertura da batedeira; (6) a aba "Visão geral" do operador deve priorizar métricas reais do negócio (ex.: lucro do dia, entregas e pedidos aceitos) em vez de conteúdo genérico. | O responsável pelo projeto testou o protótipo publicado e listou esses pontos como necessários para que a próxima rodada de validação com participantes não seja prejudicada por uma interface pouco intuitiva ou que pareça artificial. O item (4) já estava implementado antes do feedback (commit `034bd5b`, 17/09/2026 13h06, anterior ao feedback registrado às 19h43) e foi apenas confirmado, sem alteração adicional. O item (1) foi aplicado nesta rodada de forma parcial e deliberadamente limitada: banners e textos de interface foram reescritos para remover o rótulo "fictício/simulado/demonstração"; identificadores internos de ator usados em lógica de auditoria e correspondência de eventos (ex.: `"Administrador da demonstração"`, `"Cliente da demonstração"`) foram mantidos nesta rodada por estarem acoplados a regras de negócio e trilha de auditoria, para não introduzir risco de regressão fora do escopo pedido; ficam como pendência explícita para uma revisão dedicada. Detalhes técnicos da implementação em `prototypes/web/VERIFICATION.md`. |
