...
import { cents, formatOrderDate, getOrderStatusLabel, type Order } from '@/lib/mock-data'
...
export function CustomerOrder({ order }: { order: Order }) {
  ...
  return <article className="space-y-4 rounded-2xl border bg-card p-4 sm:p-6">
    ...
    <div className="flex flex-wrap gap-2">{order.status === 'AGUARDANDO_ACEITE' && <button className="demo-button" disabled={remaining === 0} onClick={() => setCancel(true)}>Cancelar pedido</button>}<SupportButton orderId={order.id} /></div>
    {cancel && <Modal title="Cancelar este pedido?" onClose={() => setCancel(false)}><p className="mb-4">O pedido ainda não foi aceito. O cancelamento não poderá ser desfeito.</p><div className="flex flex-wrap gap-2"><button className="demo-button" onClick={() => setCancel(false)}>Manter pedido</button><button className="demo-primary" onClick={() => { try { demo.transition(order.id, 'CANCELADO', { role: 'cliente', name: demo.customer.name || 'Cliente da demonstração', customerId: 'customer-001' }); setCancel(false); notify('Pedido cancelado.') } catch (error) { setCancel(false); notify(error instanceof Error ? error.message : 'Não foi possível cancelar.', 'error') } }}>Confirmar cancelamento</button></div></Modal>}
  </article>
}
